...
//
// physicalToFibonacci[] === physical LED order to ordered by radius (vogel spiral order)
// fibonacciToPhysical[] === radius (vogel spiral order) to physical LED order
// angles[]              === angle, where the range is from 0..255 for entire circle
...
// ]]
//
// Once all the LEDs actual positions are calculated, transform the original
// coordinates to fit the uint8_t coordinates used:
//
// 1. Translate the spiral so minimum X value is zero (shift right)
// 2. Translate the spiral so minimum Y value is zero (shift up)
// 3. Scale the resulting spiral so maximum X / Y value is 255.
// 
// The resulting coordinates are then stored in the coordsX/Y ranges,
// to allow for faster (8-bit) math operations on lower-power MCUs.
//
