...
// TODO: only Fib256/Fib512 need first two arrays to be uint16_t
//       consider defining as uint8_t vs. uint16_t, and updating
//       dependent code to infer type at compilation time?  This
//       may save a few cycles on the QT Py and similar MCUs....
//       For now, just upgrading them all to use `uint16_t`...
  ...
  // 512 pixels, but some calculations require range 0..255.
  // Therefore, define new array with each element == `physicalToFibonacci[i] / 2`
  ...
  // with <= 256 pixels, can create an array reference variable (zero flash/ram cost)
  const uint8_t  (&radii)[NUM_PIXELS] = physicalToFibonacci;
